...
const { User, Role } = require('./Social/User');
...
// Colaboradores
Galeria.belongsToMany(User, { through: GaleriaContributor, foreignKey: 'galleryId', otherKey: 'userId', as: 'collaborators' });
User.belongsToMany(Galeria, { through: GaleriaContributor, foreignKey: 'userId', otherKey: 'galleryId', as: 'shared_galleries' });
...
User.belongsToMany(User, { through: Rodinha, as: 'rodinhaMembers', foreignKey: 'userId', otherKey: 'targetUserId' });
User.belongsToMany(User, { through: Rodinha, as: 'inRodinhas', foreignKey: 'targetUserId', otherKey: 'userId' });
...
DMMessage.belongsTo(User, { foreignKey: 'userId', as: 'Sender' });
...
// Associações de Pet, Item e Inventário
Pet.belongsTo(User, { foreignKey: 'userId', as: 'owner' });
User.hasOne(Pet, { foreignKey: 'userId' });

Pet.hasMany(PetInventory, { foreignKey: 'petId', as: 'inventory' });
PetInventory.belongsTo(Item, { foreignKey: 'itemId', as: 'item' });
Item.hasMany(PetInventory, { foreignKey: 'itemId', as: 'inventories' });
...
User.belongsToMany(Badge, { through: BadgeUser, foreignKey: 'userId', otherKey: 'badgeId', as: 'badges' });
Badge.belongsToMany(User, { through: BadgeUser, foreignKey: 'badgeId', otherKey: 'userId', as: 'owners' });
BadgeUser.belongsTo(Badge, { foreignKey: 'badgeId' });
BadgeUser.belongsTo(User, { foreignKey: 'userId' });

// Associações de Cutucada
Cutucada.belongsTo(User, { foreignKey: 'senderUserId', as: 'sender' });
Cutucada.belongsTo(User, { foreignKey: 'targetUserId', as: 'target' });
